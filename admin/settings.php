...
function generateFlushKey(): string {
    ...
    // Cryptographically-safe shuffle using random_int().
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['flush_history'])) {
    ...
    if (!hash_equals($_SESSION['flush_confirmation_key'], $confirmation)) {
        ...
        try {
            ...
            // Reset AUTO_INCREMENT values so new records start again from 1.
            ...
            // Generate a fresh key after a successful flush.
            $_SESSION['flush_confirmation_key'] = generateFlushKey();
            $flushKey = $_SESSION['flush_confirmation_key'];

        } catch (Throwable $e) {
            $conn->rollback();
            $error = 'Unable to flush service history: ' . $e->getMessage();
...
if ($result) {
    $serviceCount = (int)$result->fetch_assoc()['total'];
}

$result = $conn->query("SELECT COUNT(*) AS total FROM request_updates");
if ($result) {
    $updateCount = (int)$result->fetch_assoc()['total'];
}
?>
<!doctype html>
<html lang="en">
<head>
  ...
  <title>System Tools | RamTech</title>
  ...
  <div class="mx-auto max-w-5xl">
    ...
    <div>
      ...
      <p class="mt-2 max-w-2xl text-black/55">
        Administrative maintenance tools for the RamTech service management system.
      </p>
    </div>
    ...
    <div class="mt-8 grid gap-5 sm:grid-cols-2">
      <div class="rounded-2xl border border-black/10 bg-white/45 p-6">
        <div class="text-4xl font-black"><?= number_format($serviceCount) ?></div>
        <div class="mt-2 text-sm text-black/55">Service request records</div>
      </div>

      <div class="rounded-2xl border border-black/10 bg-white/45 p-6">
        <div class="text-4xl font-black"><?= number_format($updateCount) ?></div>
        <div class="mt-2 text-sm text-black/55">Request update records</div>
      </div>
    </div>
    ...
    <section class="mt-8 overflow-hidden rounded-3xl border border-red-200 bg-white/55">
      <div class="border-b border-red-200 bg-red-50 p-6">
        <p class="text-xs font-black uppercase tracking-[0.25em] text-red-600">
          Danger Zone
        </p>

        <h2 class="mt-2 text-2xl font-black text-red-800">
          Flush History
        </h2>

        <p class="mt-2 max-w-3xl text-sm leading-6 text-red-700/80">
          Permanently deletes all service request records and all associated request update history.
          Client and administrator accounts are preserved.
        </p>
      </div>

      <form method="post" class="p-6"
            onsubmit="return confirm('This will permanently delete ALL service request history. This action cannot be undone. Continue?');">
        ...
        <!-- Random confirmation key -->
        <div class="mt-6 max-w-md rounded-2xl border border-black/10 bg-[#121312] p-5 text-white">
          <div class="text-xs font-bold uppercase tracking-[0.22em] text-white/45">
            Confirmation Key
          </div>

          <div class="mt-3 select-all font-mono text-3xl font-black tracking-[0.18em]">
            <?= e($flushKey) ?>
          </div>

          <p class="mt-3 text-xs leading-5 text-white/45">
            Enter this randomly generated key exactly as shown below.
          </p>
        </div>
        ...
        <button
          ...
          name="flush_history"
          value="1"
          class="mt-6 rounded-full bg-red-700 px-6 py-3 font-bold text-white hover:bg-red-800">
          Flush History
        </button>
      </form>
    </section>
